...
from fftLayer import FFTLayer2D

data_dir = pathlib.Path("FFTDifference")
...
resultsFile = open("fft_results_single_layer.txt", "w+")
        ...
        for neuralCount in hiddenLayer:
            for percentage in dropOut:
                ...
                model = tensorflow.keras.Sequential(
                    [
                        tensorflow.keras.layers.Rescaling(1./65535, input_shape=(img_height, img_width, 1)),
                ...
                imageName = "fft_1_"+str(iteration)+".png"
                plt.savefig(imageName)

                results = str(firstLayerConv) + ", " + str(firstLayerPool) + ", " + str(secondLayerConv) + ", " + str(secondLayerPool) +  ", " \
                        + ", " + str(neuralCount) + ", " + str(percentage) + ", " + "{:.4f}".format(acc[-5]) + ", " + "{:.4f}".format(loss[-5]) + ", " \
                        + "{:.4f}".format(val_acc[-5]) + ", " + "{:.4f}".format(val_loss[-5]) + ", " + str(len(acc)) + ", " + imageName + "\n"
